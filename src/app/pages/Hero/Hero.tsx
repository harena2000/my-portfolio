...
/* Use pure CSS animation for floating badges instead of Framer Motion infinite loops */
...
function HeroInner() {
  ...
  useEffect(() => {
    ...
    function animate() {
      if (!isActiveRef.current) { rafRef.current = 0; return }
    ...
    >
      ...
      <div className="w-full max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-x-12 items-center z-10">
        ...
        >
          ...
          {/* Name */}
          <motion.div variants={itemVariants}>
            <div className="flex items-start gap-3 md:gap-4 justify-center md:justify-start mb-1">
              <Image
                src="/logo.png"
                alt="HR Logo"
                width={120}
                height={120}
                className="w-[60px] h-[60px] sm:w-[76px] sm:h-[76px] md:w-[96px] md:h-[96px] lg:w-[120px] lg:h-[120px] object-contain shrink-0 drop-shadow-[0_0_12px_rgba(59,130,246,0.5)]"
              />
        ...
        >
          ...
          {/* Profile photo — CSS float animation instead of Framer Motion infinite */}
